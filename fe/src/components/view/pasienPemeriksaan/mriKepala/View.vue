...
import { usePemeriksaanMriKepalaStore } from '@/stores/module/pemeriksaanMriKepala'
...
const mriStore = usePemeriksaanMriKepalaStore()
const table = ref()
const formModal = ref()
...
function handleBtnDelete(id: number): void {
    mriStore.actionDelete(id).then(() => {
        table.value.reload()
    })
}

function actionUpSert(data: any): void {
    mriStore.actionUpSertFile(data).then(() => {
