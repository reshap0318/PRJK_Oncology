<template>
    ...
    <DataTable
        ref="table"
        :url="boneSurveyStore.datatableLink"
...
import { usePemeriksaanBoneSurveyStore } from '@/stores/module/pemeriksaanBoneSurvey'
...
const boneSurveyStore = usePemeriksaanBoneSurveyStore()
const table = ref()
const formModal = ref()
...
function handleBtnDelete(id: number): void {
    boneSurveyStore.actionDelete(id).then(() => {
        table.value.reload()
    })
}

function actionUpSert(data: any): void {
    boneSurveyStore.actionUpSertFile(data).then(() => {
