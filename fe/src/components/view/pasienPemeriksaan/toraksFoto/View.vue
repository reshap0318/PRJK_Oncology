<template>
    <div class="d-flex justify-content-between align-items-center mb-4">
        <h3 class="mb-0">Ro. Foto Toraks</h3>
    ...
    <DataTable
        ref="table"
        :url="toraksFotoStore.datatableLink"
...
import { usePemeriksaanToraksFotoStore } from '@/stores/module/pemeriksaanToraksFoto'
...
const toraksFotoStore = usePemeriksaanToraksFotoStore()
const table = ref()
const formModal = ref()
...
function handleBtnDelete(id: number): void {
    toraksFotoStore.actionDelete(id).then(() => {
        table.value.reload()
    })
}

function actionUpSert(data: any): void {
    toraksFotoStore.actionUpSertFile(data).then(() => {
