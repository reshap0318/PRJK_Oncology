...
use App\Repository\Module\PemeriksaanBoneSurveyRepository;
use App\Services\BaseService;
use Illuminate\Support\Facades\Storage;
use Yajra\DataTables\Facades\DataTables;

class PemeriksaanBoneSurveyService extends BaseService
{
    protected $mainRepository;
    public function __construct()
    {
        $this->mainRepository = new PemeriksaanBoneSurveyRepository();
    }
    ...
    public function create($payload)
    {
        ...
        if(isset($payload['file']) && $payload['file']->isValid()) {
            $fileName = $data->id . "-bone-survey." . $payload['file']->extension();
            $data->update([
                'file_path' => $payload['file']->storeAs('bone-survey', $fileName)
            ]);
        }
        return $data;
    }
    ...
    public function update($id, $payload)
    {
        ...
        if(isset($payload['file']) && $payload['file']->isValid()) {
            $fileName = $data->id . "-bone-survey." . $payload['file']->extension();
            $payload['file_path'] = $payload['file']->storeAs('bone-survey', $fileName);
        }
